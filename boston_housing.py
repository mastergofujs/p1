...
def cross_val_score(estimator, X, y, scoring=performance_metric, cv=3):
    ...
    for i in range(0, length-1, split_width):
        ...
        if i + split_width < length-1:
            X_batch = pd.concat([X_batch,X.ix[i:i + split_width,:]])
            y_batch = pd.concat([y_batch,y.ix[i:i + split_width]])
        else:
            X_batch = pd.concat([X_batch, X.ix[length-split_width:length, :]])
            y_batch = pd.concat([y_batch, y.ix[length-split_width:length]])
    ...
    for i in range(cv):
        ...
        scores.append(scoring(X_test, y_test))
    return scores
...
def fit_model2(X, y):
    ...
    for max_depth in range(1, 11):
        ...
        scores[max_depth] = np.mean(cross_val_score(reg, X, y, scoring=performance_metric, cv=3))
        models.append(reg)
    # 最优交叉验证分数对应的最优模型
